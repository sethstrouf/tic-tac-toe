...
class Controller
  ...
  def get_input
    ...
    letter = test_input
    [" #{letter} ", @player_to_go.name]
  end

  def switch_player_turn
    @player_to_go = if @player_to_go == @players[0]
                      @players[1]
                    else
                      @players[0]
                    end
  ...
  def test_input
    ...
    while test_passed == false
      ...
      if (letter >= 'A') && (letter <= 'I') && !@stored_guesses.include?(letter)
        test_passed = true
...
# Creates new game with 2 players and a grid with 3 squares per side
game = Game.new(2, 3)
game.start_game
